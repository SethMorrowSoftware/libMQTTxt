...
def strip_noise(text):
    """Blank comments and string literals, tracking string state line by line.

    Inlined rather than imported: the suite borrows this from
    check-handler-calls.py, which this repo does not carry. A `--` INSIDE a
    literal is not a comment, and getting that backwards has silently changed
    a scan's answer three separate times in this family - so the state machine
    is explicit.

    NOTE the deliberate asymmetry with main(): the ARMED names are read from
    RAW text, because blanking literals turns `send "mdDashOnce" to me in`
    into `send "" to me in` and the scan finds nothing while reporting clean.
    """
    out = []
...
def main(argv):
    ...
    for path in sorted(set(sources)):
        ...
        if rel.startswith(".git"):
            continue
        # The masters under tools/ are references, not adopters: their carried
        # copies inside the demo are what actually runs, and those are scanned.
